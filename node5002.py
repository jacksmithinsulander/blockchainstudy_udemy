...
def is_valid():
    ...
    if is_valid:
        response = {'message': 'All good in the hood. The Blockchain is valid'}
    else:
...
def connect_node():
    ...
    response = {'message': 'Nodes connected. Cowboycoin now contains nodes:' ,
            'total_nodes': list(blockchain.nodes)}
    return jsonify(response), 201

#Replace to longest chain if needed
@app.route('/replace_chain', methods = ['GET'])
def replace_chain():
    is_chain_replaced= blockchain.replace_chain()
    if is_chain_replaced:
        response = {'message': 'Differentiating nodes. Chain replaced by longest chain',
                'new_chain': blockchain.chain}
    else:
        response = {'message': 'All good. This is the longest chain',
